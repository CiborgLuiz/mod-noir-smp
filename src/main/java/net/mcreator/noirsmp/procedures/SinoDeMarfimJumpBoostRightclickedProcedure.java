...
public class SinoDeMarfimJumpBoostRightclickedProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if (!(entity instanceof Player _plrCldCheck0 && _plrCldCheck0.getCooldowns().isOnCooldown(NoirSmpModItems.SINO_DE_MARFIM_JUMP_BOOST.get()))) {
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
				_entity.addEffect(new MobEffectInstance(MobEffects.JUMP, 2400, 2, false, false));
			if (entity instanceof Player _player)
				_player.getCooldowns().addCooldown(NoirSmpModItems.SINO_DE_MARFIM_JUMP_BOOST.get(), 4800);
