...
import net.mcreator.noirsmp.procedures.MiniBuracoNegroProjectileHitsBlockProcedure;
...
public class MiniBuracoNegroEntity extends AbstractArrow implements ItemSupplier {
	...
	public void onHitEntity(EntityHitResult entityHitResult) {
		...
		MiniBuracoNegroProjectileHitsBlockProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}

	@Override
	public void onHitBlock(BlockHitResult blockHitResult) {
		super.onHitBlock(blockHitResult);
		MiniBuracoNegroProjectileHitsBlockProcedure.execute(this.level(), blockHitResult.getBlockPos().getX(), blockHitResult.getBlockPos().getY(), blockHitResult.getBlockPos().getZ());
	}
	...
	public static MiniBuracoNegroEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		MiniBuracoNegroEntity entityarrow = new MiniBuracoNegroEntity(NoirSmpModEntities.MINI_BURACO_NEGRO.get(), entity, world);
		...
		world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("noir_smp:buraco_negro")), SoundSource.PLAYERS, 1, 1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}

	public static MiniBuracoNegroEntity shoot(LivingEntity entity, LivingEntity target) {
		MiniBuracoNegroEntity entityarrow = new MiniBuracoNegroEntity(NoirSmpModEntities.MINI_BURACO_NEGRO.get(), entity, entity.level());
		...
		entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("noir_smp:buraco_negro")), SoundSource.PLAYERS, 1, 1f / (RandomSource.create().nextFloat() * 0.5f + 1));
		return entityarrow;
	}
}